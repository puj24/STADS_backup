                    ...
                    else
                    {
...

                        {
                            ...
                            else
                            {
                                ...
                                arr_out_img[j_set_tags][i_set_tags][1] = *tag_num_ptr;
...
//arr_star_coordinates has been declared under the global scope to prevent stack overflow
    ...
    //if the number of zero rows between dense rows is positive
        if (num_zero_rows > 0)
        {
            ...
            for (i_remove_zero_rows = (num_final_tags - num_zero_rows); i_remove_zero_rows < (num_stars + 1); i_remove_zero_rows++)
            {
            ...
            for (i_overwrite_rows = (num_stars + 1); i_overwrite_rows < (num_stars + num_zero_rows + 1); i_overwrite_rows++)
            {
...
//unsigned short int *num_final_tags_ptr= &num_final_tags;
    ...
    // getting interediate output from tag_2 and merge_tag_3
        ...
        for(i_set_star_id = 0; i_set_star_id < num_stars; i_set_star_id++)
        {
